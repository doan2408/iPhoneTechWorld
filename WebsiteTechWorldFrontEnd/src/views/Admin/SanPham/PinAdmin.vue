searchQuery<template>
    <div class="container mt-4">
        ...
        <div class="table-responsive mb-4" style="margin-top: 20px;">
            <el-table :data="dataPin" border style="width: 100%">
                <el-table-column type="index" :index="indexMethod" label="STT" width="80" />
                <!-- <el-table-column prop="id" label="STT" /> -->
                <el-table-column prop="phienBan" label="Phiên bản" />
                <el-table-column prop="congSuatSac" label="Công suất" />
                <el-table-column prop="thoiGianSuDung" label="Thời gian sử dụng" />
                <el-table-column prop="soLanSacToiDa" label="Số lần sạc tối đa" />
        ...
        <el-dialog v-model="dialogVisible" :title="isEditMode ? 'Chỉnh sửa pin' : 'Thêm mới pin'" width="900px"
...
<script setup>
import { deletePin, getAllPinPage, postPin, putPin } from '@/Service/Adminservice/Products/ProductAdminService';
...
const loadData = async () => {
    try {
        const response = await getAllPinPage(currentPage.value - 1, pageSize, searchQuery.value);
        dataPin.value = response.content;
...
const handleDelete = async (id) => {
    try {
        await ElMessageBox.confirm(
            'Bạn có chắc chắn muốn xoá pin này không?',
            'Xác nhận xoá',
            {
